...
class PuzzleImageModel(nn.Module):
    ...
    def forward(self, imgs):
        ...
        cropped, img_indices = self.crop_images(results, imgs)
    ...
    def crop_images(self, results, imgs):
        """
            ...
            Returns:
                tuple: tensor of batched cropped and images list of indices
        """
        all_crops = []
        image_indices = []
        ...
        return torch.stack(all_crops), image_indices
                
    def forward_until_layer(self, imgs, layer: int):
        """
            Compute forward pass of the model until the specified layer
            Args:
                imgs: list of images to compute forward pass on
                layer: layer to return embeddings from
            Returns:
                features: list of embeddings
        """
        features = []
        ...
        with torch.no_grad():
            _ = self.model(imgs, verbose=False)
        hook.remove()
