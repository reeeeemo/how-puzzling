...
class PuzzleDataset(Dataset):
    ...
    def __init__(self,
        ...
        if len(splits) <= 0:
            warnings.warn(("Invalid splits given."
                          "Please instantiate with at least 1 split"))
            return

        # check to ensure equality / exist
        self.images, self.labels = {}, {}
        self.splits = splits
        for split in splits:
            self.images[split] = sorted(
                glob(str(self.root_dir / "images" / split / "**"
                         / f"*.{extension}"), recursive=True)
            )
            self.labels[split] = sorted(
                glob(str(self.root_dir / "labels" / split / "**"
                         / "*.txt"), recursive=True)
            )
            ...
            if len_image != len_lbl:
                warnings.warn((
                    f"{split} Image ({len_image}) and Label"
                    f"({len_lbl}) directories have unequal elements.")
                )
            if not img_path or not lbl_path:
                warnings.warn((
                    f"{split} Image path exists: ({bool(img_path)})."
                    f"Label path exists ({bool(lbl_path)})")
                )
    ...
    def __getitem__(self, idx: int):
        ...
        # contrast limited adaptive histogram equalization to improve contrast
                ...
                for line in f.readlines():
                    ...
                    if len(values) != 5:
                        warnings.warn((
                            ...
                            f"expected 5 vals, got {len(values)}")
                        )
                        continue
                    labels.append(torch.tensor([float(v) for v in values]))
        except Exception as e:
            warnings.warn(
                f"Unexpected error occured for file {label_path}: {e}"
            )
            return image, []
